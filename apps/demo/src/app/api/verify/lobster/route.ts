...
// POST /api/verify/lobster — Verify Lobster identity via X post code
    ...
    try {
        ...
        if (!xHandle || typeof xHandle !== "string") {
            return NextResponse.json({
                status: "pending",
                instructions: [
                    `1. Post this to X (Twitter): "Joining @PrometheusAvatar as a 🦞 OpenClaw Lobster! My verification code: ${verificationCode} #PrometheusAvatar #OpenClaw"`,
        ...
        return NextResponse.json({
            ...
            xHandle: `@${cleanHandle}`,
            verificationCode,
            message: `🦞 Lobster identity verified for @${cleanHandle}! Welcome to Prometheus.`,
        });
