...
// GET /api/migrate/airachne?email=xxx — Preview conversion
export async function GET(req: NextRequest) {
    ...
    if (existing?.is_airachne_user) {
        return NextResponse.json({
            ...
            message: "This account has already been migrated from Airachne.",
        });
    }
...
// POST /api/migrate/airachne — Execute migration
    ...
    try {
        ...
        if (account) {
            await supabase.from("point_transactions").insert({
                ...
                type: "airachne_migration",
                description: `Migrated ${airBalance} $AIR → ${prometheusPoints} Prometheus pts (tiered)`,
        ...
        return NextResponse.json({
            ...
            message: lifetimeGranted
                ? `🎉 Migrated ${airBalance} $AIR → ${prometheusPoints} pts + Lifetime Membership!`
                : `✅ Migrated ${airBalance} $AIR → ${prometheusPoints} Prometheus points.`,
        });
